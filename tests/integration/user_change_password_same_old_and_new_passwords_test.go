...
func TestUserChangePasswordSameOldAndNewPasswordsFailed(t *testing.T) {
	...
	// 3. Change Password
	result, response := helper.ChangePassword(t, &client, server, testingVariables, testingVariables.Password)
	if result {
		t.Fatalf(
			"Change password passed with same old and new passwords user failed. Email %s, password %s",
		...
		if errorMessage.ErrorMessage == service.ErrUserChangePasswordFailed.Error() {
			t.Logf(
				"✅ Change user password with same old and new passwords test passed. Error message: %s",
				errorMessage.ErrorMessage,
			)
		} else {
			t.Fatalf(
				"Change user password with same old and new passwords failed. Email %s, message %s",
				testingVariables.Email,
				errorMessage.ErrorMessage,
			)
		}
